...
impl GitLabClient {
    ...
    pub async fn latest_pipeline_for_branch(
        ...
        if !status.is_success() {
            return Err(
                anyhow::Error::from(HttpStatusError { status }).context(format!(
...
pub struct MergeRequest {
    ...
    /// The head commit of the merge request. Empty when GitLab reported no
    /// head, e.g. a transitional MR whose source branch is gone or whose
    /// diff has not been generated yet.
    pub sha: String,
...
mod tests {
    ...
    fn associated_commit_mrs_preserve_head_sha_and_merge_state() {
        let mr: MergeRequest = GitLabMergeRequest {
            ...
            sha: Some("1234567890abcdef1234567890abcdef12345678".into()),
